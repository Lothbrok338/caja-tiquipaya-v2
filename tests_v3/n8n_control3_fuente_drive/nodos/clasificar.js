// Exige EXACTAMENTE un GLOBAL oficial; los historicos MAESTROS (raiz de 05_CONTROLES) y el reporte previo del periodo
// (carpeta <YYYY-MM>) son opcionales (0 o 1), nunca mas de uno. Los snapshots por periodo nunca se buscan aqui.
...
const g = exactos('BUSCAR GLOBAL oficial CONTROL3 (Drive)', rc.nombre_global);
...
if (g.length === 0) {
  throw new Error('GLOBAL_OFICIAL_NO_ENCONTRADO: no existe "' + rc.nombre_global + '" en 05_CONTROLES/GLOBAL de Drive. CONTROL 3 no usa un GLOBAL local como sustituto: genera el GLOBAL antes.');
}
if (g.length > 1) {
