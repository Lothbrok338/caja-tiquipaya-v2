...
en Code nodes, las 53 sustituciones $env.TIQ_BASE_DIR esperadas,
...
def test_53_sustituciones_de_process_env_quedan_contabilizadas():
    ...
    process.env) se reparte ahora en dos variables: 51 en
    $env.TIQ_BASE_DIR (la ruta base genérica) y 2 en
    ...
    — las 2 que antes también caían dentro de esas 53 porque el literal
    de la plantilla contenía el mismo prefijo /home/codespace/.n8n-files.
    El total sigue siendo 53: ninguna ocurrencia se pierde."""
    archivos = [os.path.join(DST_DIR, BACKEND), os.path.join(DST_DIR, INGESTA)]
