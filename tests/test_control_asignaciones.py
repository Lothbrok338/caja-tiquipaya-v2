...
# ---------------------------------------------------------------------------
# 12-14: exclusiones — SIN CAMBIOS respecto de la versión anterior
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 1: mes sin duplicados -> histórico se actualiza normalmente
# ---------------------------------------------------------------------------

class TestSinDuplicados(_ControlAsignacionesTestBase):
    def test_mes_sin_duplicados_actualiza_historico(self):
...
# ---------------------------------------------------------------------------
# 2-4: alertas sin validar (o parcialmente validadas) -> no actualiza
# ---------------------------------------------------------------------------

class TestPendienteValidacion(_ControlAsignacionesTestBase):
    def test_duplicado_mismo_mes_sin_validar_no_actualiza_historico(self):
        resumen = self._ejecutar([_partida("ASIG-DUP"), _partida("ASIG-DUP")])
        self.assertEqual(resumen["estado"], "PENDIENTE_VALIDACION_AUDITOR")
    ...
    def test_duplicado_contra_historico_sin_validar_no_actualiza_historico(self):
        ...
        resumen = self._ejecutar([_partida("ASIG-HIST")])
        self.assertEqual(resumen["estado"], "PENDIENTE_VALIDACION_AUDITOR")
        self.assertFalse(resumen["historico_actualizado"])
        revision = _leer_csv(self._ruta_revision("AGOSTO_2026"))
        self.assertEqual(revision[0]["TIPO_ALERTA"], "DUPLICADA_CON_HISTORICO")
...
# ---------------------------------------------------------------------------
# 5-6: todas las alertas validadas -> cierre e incorporación al histórico
# ---------------------------------------------------------------------------

class TestCierreConValidacion(_ControlAsignacionesTestBase):
    def test_todas_correctas_actualiza_historico(self):
        ...
        resumen = ca.ejecutar_control(ruta_global=ruta_global, ruta_historico=self.ruta_historico)
        self.assertEqual(resumen["estado"], "CERRADO_CON_VALIDACION_AUDITOR")
        self.assertTrue(resumen["historico_actualizado"])
        self.assertEqual(resumen["filas_incorporadas_historico"], 2)
...
# ---------------------------------------------------------------------------
# 7-8: una validación anterior NUNCA evita una alerta futura; se conserva
# la observación humana como antecedente.
# ---------------------------------------------------------------------------

class TestNoEvitaAlertaFutura(_ControlAsignacionesTestBase):
    def test_validada_correcta_en_agosto_vuelve_a_alertar_en_septiembre(self):
        ...
        self.assertEqual(resumen_agosto["estado"], "CERRADO_CON_VALIDACION_AUDITOR")
        ...
        self.assertEqual(resumen_sept["estado"], "PENDIENTE_VALIDACION_AUDITOR")
...
# ---------------------------------------------------------------------------
# 9: dry-run nunca escribe (ni revisión ni histórico)
# ---------------------------------------------------------------------------

class TestDryRun(_ControlAsignacionesTestBase):
    def test_dry_run_no_escribe_revision_ni_historico(self):
    ...
    def test_dry_run_sin_duplicados_tampoco_escribe_historico(self):
...
# ---------------------------------------------------------------------------
# 10-11: idempotencia — sin cambios respecto de la versión anterior
# ---------------------------------------------------------------------------
...
class TestIdempotencia(_ControlAsignacionesTestBase):
    ...
    def test_mismo_archivo_global_sha_diferente_bloquea(self):
...
# ---------------------------------------------------------------------------
# 15: histórico con esquema antiguo (sin columnas de validación) se lee y
# se reescribe sin perder información.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Preservación de hallazgos "por par" (compatibilidad del JSON de detalle)
# ---------------------------------------------------------------------------
