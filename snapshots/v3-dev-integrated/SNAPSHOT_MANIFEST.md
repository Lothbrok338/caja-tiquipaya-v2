# SNAPSHOT_MANIFEST.md — TIQ V3 DEV integrada (post-FASE 8)

Fecha de captura: **2026-09-14T04:29:44Z**

Este snapshot congela el estado de los 8 workflows de Caja Tiquipaya V3
inmediatamente después de FASE 8 (integración end-to-end 01→07 en DEV,
ejecución manual real verificada vía n8n `test_workflow`, execution #157,
`status: success`). Ninguno de estos workflows está activo; ninguno usa
Drive/SAP/marcadores productivos. **No contiene credenciales ni secretos**
— cada JSON es exactamente el objeto workflow devuelto por
`get_workflow_details`, sin el bloque `scopes`/`canExecute`/`triggerInfo`
(metadata de lectura de la API, no parte del workflow en sí).
...
Los 8 workflows: `active=false`, `activeVersionId=null`, `triggerCount=0` —
ninguno está activo ni tiene webhooks/triggers productivos registrados.
...
- `~/.n8n-files/tiq_v3_manual_dev/` (fixtures y outputs DEV de la ejecución
  manual — regenerables con `tests/xlsx_fixtures.py`, no versionados).
- Archivos temporales, cachés, `__pycache__`, `.pytest_cache`.
- Credenciales o secretos de ningún tipo (ninguno de los 8 workflows usa
  credenciales — todos son `Code`/`executeCommand`/`readWriteFile`/
  `extractFromFile`/`executeWorkflow`, sin nodos de credenciales).
